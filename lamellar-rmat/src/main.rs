...
use lamellar::{darc::Darc, typed_am_group, ActiveMessaging};
use std::io::prelude::*;
...
fn main() {
    ...
    let gen = DistRMAT::<SmallRng>::new(&world, 18, 0.1, Some(0), 100000, [0.57, 0.19, 0.19, 0.05], false);
    let gen = Darc::new(&world, gen).unwrap();
    if my_pe == 0 {
        let edges: Vec<_> = world.block_on(world.exec_am_all(Yoink {gen})).into_iter().flatten().collect();
        let f = std::fs::File::options().write(true).truncate(true).open("graph.json").unwrap();
